...
NSString * const XPPreferencesDidChangeNotification = @"XPPreferencesDidChangeNotification";
NSString * const XPPreferencesChangedPropertyKey = @"XPPreferencesChangedPropertyKey";

static NSMutableDictionary *_dynamicProperties;


NS_INLINE XPPropertyDescriptor * propertyDescriptorForSelector(SEL _cmd) {
    NSString *selectorString = NSStringFromSelector(_cmd);
    XPPropertyDescriptor *descriptor = _dynamicProperties[selectorString];
    
    return descriptor;
}

NS_INLINE NSString * defaultsKeyForSelector(SEL _cmd) {
    XPPropertyDescriptor *descriptor = propertyDescriptorForSelector(_cmd);
...
void paprefBoolSetter(id self, SEL _cmd, BOOL value) {
    ...
    NSDictionary *userInfo = @{XPPreferencesChangedPropertyKey: defaultsKey};
    [[NSNotificationCenter defaultCenter] postNotificationName:XPPreferencesDidChangeNotification object:self userInfo:userInfo];
}
...
void paprefURLSetter(id self, SEL _cmd, NSURL *value) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:XPPreferencesDidChangeNotification object:self];
}
...
@implementation XPPreferences

+ (instancetype)sharedInstance {
    static XPPreferences *_sharedInstance;
...
+ (NSSet *)keyPathsForValuesAffectingValueForKey:(NSString *)key
{
    ...
    XPPropertyDescriptor *propertyDescriptor = _dynamicProperties[key];
            ...
            if (isDynamic) {
                if (shouldRetain) {
                    ...
                    if ([self isValidType:type]) {
                        ...
                        _dynamicProperties[getterName] = [[XPPropertyDescriptor alloc] initWithDefaultsKey:defaultsKey type:type isSetter:NO];
                        _dynamicProperties[setterName] = [[XPPropertyDescriptor alloc] initWithDefaultsKey:defaultsKey type:type isSetter:YES];
                    } else {
...
+ (BOOL)resolveInstanceMethod:(SEL)sel {
    ...
    XPPropertyDescriptor *propertyDescriptor = _dynamicProperties[selectorString];
